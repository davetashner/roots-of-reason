...
func _on_pandemic_started(player_id: int, _severity: float) -> void:
	if _pandemic_vfx != null:
		var tc_pos := _find_town_center_pos(player_id)
...
func _setup_historical_events() -> void:
	_historical_event_manager = Node.new()
	_historical_event_manager.name = "HistoricalEventManager"
	_historical_event_manager.set_script(HistoricalEventManagerScript)
	add_child(_historical_event_manager)
	_historical_event_manager.setup(_population_manager, _tech_manager, _trade_manager, _building_placer, self)
	_historical_event_manager.event_started.connect(_on_hist_event_started)
	_historical_event_manager.event_ended.connect(_on_hist_event_ended)


func _on_hist_event_started(event_id: String, player_id: int) -> void:
	if _historical_event_vfx == null:
		return
	var pos := _find_town_center_pos(player_id)
	match event_id:
		"black_plague":
			_historical_event_vfx.play_plague_effect(pos, player_id)
		"renaissance":
			if _historical_event_manager.is_phoenix_active(player_id):
				_historical_event_vfx.play_phoenix_effect(pos, player_id)
			else:
				_historical_event_vfx.play_renaissance_effect(pos, player_id)
...
func _on_hist_event_ended(event_id: String, player_id: int) -> void:
	...
	match event_id:
		"black_plague":
			_historical_event_vfx.play_plague_end(player_id)
		"renaissance":
			_historical_event_vfx.play_renaissance_end(player_id)
...
func _setup_hud() -> void:
	...
	_historical_event_vfx = Node.new()
	_historical_event_vfx.name = "HistoricalEventVFX"
	_historical_event_vfx.set_script(HistoricalEventVFXScript)
	add_child(_historical_event_vfx)
	_historical_event_vfx.setup(self, _camera, _notification_panel)
